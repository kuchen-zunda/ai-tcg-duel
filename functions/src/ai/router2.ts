...
function safe<T=any>(s:string):T { try { return JSON.parse(s) } catch { return {kind:"unknown", ask:"もう一度短くお願い"} as any } }

export async function routeUserText2(
  ...
  const hand = seat==="P1" ? (priv?.hand ?? []) : []; // 今はP1のみ
